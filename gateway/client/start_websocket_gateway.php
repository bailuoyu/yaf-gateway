...
if(empty($allconfig['websocket_gateway'])){   //如果配置不存在则什么也不执行
}else{
    $congig = $allconfig['websocket_gateway'];
    
    // gateway 进程，这里使用websocket协议
    $gateway = new Gateway('websocket://'.$congig['ip'].':'.$congig['port']);
    // gateway名称，status方便查看
    $gateway->name = $appname.'_websocket_gateway';
    // gateway进程数
