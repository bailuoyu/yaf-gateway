...
if(empty($allconfig['text_gateway'])){   //如果配置不存在则什么也不执行
}else{
    $congig = $allconfig['text_gateway'];
    
    // gateway 进程，这里使用text协议
    $gateway = new Gateway('text://'.$congig['ip'].':'.$congig['port']);
    // gateway名称，status方便查看
    $gateway->name = $appname.'_text_gateway';
    // gateway进程数
